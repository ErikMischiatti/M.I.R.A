...
class SessionContextBuilder:
    """Builds a bounded, sanitized prompt context from session-local memory."""
    ...
    def build(self, current_input: UserInput | None = None) -> SessionContextSnapshot:
        ...
        sections = []
        facts_text = self._build_safe_facts_text()
        if facts_text:
    ...
    def _build_safe_facts_text(self) -> str:
        lines = []
        user_name = self.memory.get_context_value("user_name")
        if isinstance(user_name, str) and user_name.strip():
            lines.append(f"User name: {self._sanitize_text(user_name)}")
